export function InternshipCard({ internship, onToggleSave }) {
  ...
  const getSourceColor = (source) => {
    switch (source) {
      case "linkedin":
        return "text-[#0077b5] bg-[#0077b5]/10 border-[#0077b5]/30";
      case "internship_cell":
        return "text-[#4ade80] bg-[#4ade80]/10 border-[#4ade80]/30";
      default:
        return "text-[#66d7cb] bg-[#66d7cb]/10 border-[#66d7cb]/30";
    }
  };
  ...
  return (
    <div className="group rounded-2xl border border-white/15 bg-[#081f26]/85 p-5 backdrop-blur-lg transition-all hover:border-white/25 hover:shadow-lg">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <div className="mb-2 flex items-center gap-2">
            <span
              className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold capitalize ${getSourceColor(internship.source)}`}
            >
        ...
        >
          <svg
            className="h-5 w-5"
            fill={internship.saved ? "currentColor" : "none"}
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
            <path
      ...
      {eligibility?.matching_skills &&
        eligibility.matching_skills.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {eligibility.matching_skills.slice(0, 5).map((skill, i) => (
              <span
                key={i}
                className="rounded-full bg-[#4ade80]/10 px-2 py-0.5 text-xs text-[#4ade80]"
              >
                {skill}
              </span>
            ))}
